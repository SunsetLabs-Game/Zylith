...
pub fn get_liquidity_for_amount0(
    ...
    assert(sqrt_price_a_x96 < sqrt_price_b_x96, 'Invalid price range');
    
    // Calculate: amount0 * (sqrt(P_b) * sqrt(P_a)) / (sqrt(P_b) - sqrt(P_a))
    // In Q64.96 format
    
    let sqrt_price_diff = sqrt_price_b_x96 - sqrt_price_a_x96;
    assert(sqrt_price_diff > 0, 'Price diff must be positive');
    
    // numerator = amount0 * sqrt(P_b) * sqrt(P_a)
    let numerator = math::mul_u128(amount0, sqrt_price_b_x96);
    let numerator = math::mul_u128(numerator, sqrt_price_a_x96);
...
pub fn get_amount0_for_liquidity(
    ...
    let denominator = math::mul_u128(sqrt_price_b_x96, sqrt_price_a_x96);
    assert(denominator > 0, 'Denominator must be positive');
    
    // Account for Q64.96 format
    let result = numerator / denominator;
    result * 79228162514264337593543950336 / 79228162514264337593543950336; // Simplify
    
    // Calculate: amount0 = L * (sqrt(P_b) - sqrt(P_a)) / (sqrt(P_b) * sqrt(P_a))
    // Simplified for Q64.96 format
    let q96 = 79228162514264337593543950336_u128;
    let scaled_numerator = numerator * q96;
